...
export class HomeComponent implements OnInit {
  ...
  public pieChartConfig: any = {
    type: 'pie',
    data: {
      labels: ['Assault', 'Break-in', 'Vandalism', 'Theft', 'Accident', 'Heart Attack', 'Stroke', 'Breathing Issues', 'Fire', 'Explosion', 'Gas Leak', 'Flood'],
      datasets: [{
  ...
  async getAccidentCategoriesFromFirestore() {
    ...
    const counts: Record<string, number> = {
      'Assault': 0, 'Break-in': 0, 'Vandalism': 0, 'Theft': 0, 'Accident': 0,
      'Heart Attack': 0, 'Stroke': 0, 'Explosion': 0, 'Gas Leak': 0, 'Flood': 0, 'Fire': 0
    };
...
async getYearlyAccidentData() {
  ...
  querySnapshot.forEach((doc) => {
    ...
    if (timestamp) {
      const year = new Date(timestamp.seconds * 1000).getFullYear().toString(); // Extract year from timestamp
      yearCounts[year] = (yearCounts[year] || 0) + 1;  // Count events by year
    }
  });

  // Prepare the years and counts for the chart
  const years = Object.keys(yearCounts).sort((a, b) => parseInt(b) - parseInt(a)); // Sort by year descending
